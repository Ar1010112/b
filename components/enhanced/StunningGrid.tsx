    ...
    <section className="py-20 relative overflow-hidden">
      ...
      <div className={`absolute inset-0 rounded-3xl -z-10 ${
        theme === 'light' 
          ...
          : 'bg-gradient-to-b from-[#07D348]/10 to-transparent'
      }`}></div>
      
      {/* Enhanced Floating Particles - Theme Aware */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(50)].map((_, i) => (
          <motion.div
            key={i}
            className={`absolute w-1 h-1 rounded-full opacity-60 ${
              theme === 'light' ? 'bg-gray-400' : 'bg-[#07D348]'
            }`}
            style={{
      ...
      >
        <h2 className={`text-5xl font-bold mb-6 bg-clip-text text-transparent relative ${
          theme === 'light' 
            ? 'bg-gradient-to-r from-gray-800 to-[#07D348]' 
            : 'bg-gradient-to-r from-white to-[#07D348]'
        }`}>
      ...
      <div ref={gridRef} className="grid gap-8 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 relative">
        {filteredItems.map((item, index) => (
          <motion.div
            ...
            className={`grid-card group relative overflow-hidden rounded-3xl border-2 backdrop-blur-xl transition-all duration-500 hover:shadow-2xl ${
              theme === 'light'
                ? 'border-gray-300 bg-white/90 hover:border-[#07D348] hover:shadow-[#07D348]/20'
                : 'border-[#07D348]/30 bg-gradient-to-br from-black/80 to-zinc-900/60 hover:border-[#07D348]/80 hover:shadow-[#07D348]/40'
            }`}
            onMouseEnter={() => setHoveredItem(item.id)}
            onMouseLeave={() => setHoveredItem(null)}
            style={{ transformStyle: 'preserve-3d' }}
          >
            {/* Background Gradient Overlay - Theme Aware */}
            <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
              theme === 'light'
                ? 'bg-gradient-to-br from-[#07D348]/10 via-transparent to-[#24fe41]/5'
                : 'bg-gradient-to-br from-[#07D348]/20 via-transparent to-[#24fe41]/10'
            }`} />
            ...
            <div className="relative h-48 overflow-hidden rounded-t-3xl">
              ...
              <motion.div
                className={`absolute top-4 right-4 backdrop-blur-sm rounded-xl px-3 py-2 border shadow-lg ${
                  theme === 'light'
                    ? 'bg-white/90 border-gray-300'
                    : 'bg-black/90 border-[#07D348]/40'
                }`}
              ...
              >
                ...
                <div className={`text-xs ${theme === 'light' ? 'text-gray-600' : 'text-zinc-300'}`}>
                  {item.stats?.label}
                </div>
              </motion.div>

              {/* Icon Overlay - Theme Aware */}
              <motion.div
                className={`absolute bottom-4 left-4 p-3 backdrop-blur-sm rounded-xl border shadow-lg ${
                  theme === 'light'
                    ? 'bg-white/90 border-gray-300'
                    : 'bg-black/90 border-[#07D348]/40'
                }`}
            ...
            <div className="p-8 relative z-10">
              <motion.h3
                className={`text-2xl font-bold mb-4 transition-colors duration-300 ${
                  theme === 'light'
                    ? 'text-gray-800 group-hover:text-[#07D348]'
                    : 'text-white group-hover:text-[#24fe41]'
                }`}
              ...
              {/* Category Tag and Learn More - Theme Aware */}
              <div className="flex items-center justify-between">
                <span className={`px-3 py-1 rounded-full text-sm font-medium border shadow-sm ${
                  theme === 'light'
                    ? 'bg-[#07D348]/10 text-[#07D348] border-[#07D348]/30'
                    : 'bg-[#07D348]/30 text-[#24fe41] border-[#07D348]/50'
                }`}>
                  {item.category}
                </span>
            ...
            <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none">
              <div className={`absolute inset-0 rounded-3xl blur-2xl ${
                theme === 'light'
                  ? 'bg-gradient-to-r from-[#07D348]/20 to-[#24fe41]/15'
                  : 'bg-gradient-to-r from-[#07D348]/30 to-[#24fe41]/30'
              }`} />
            </div>
          </motion.div>
        ))}
